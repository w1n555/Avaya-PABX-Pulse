...
.SYNOPSIS
  ...
  prebuilt api\CmApi.dll, loopback OSSI bridge (127.0.0.1:18776), API key, scheduled task.

  ONE command for first install AND upgrade (auto-detect):
    install.bat
  data_live\monitored_trunks.json and data_live\cm_api_key.txt are kept.
...
function Get-OrCreateApiKey([string]$root) {
    $data = Join-Path $root $script:OssiDataLeaf
    New-Item -ItemType Directory -Force -Path $data | Out-Null
    $keyFile = Join-Path $data "cm_api_key.txt"
    if (Test-Path $keyFile) {
        $existing = (Get-Content -Path $keyFile -Raw -ErrorAction SilentlyContinue)
        if ($existing) {
            $existing = $existing.Trim()
            if ($existing.Length -ge 16) { return $existing }
        }
    }
    $bytes = New-Object byte[] 32
    [System.Security.Cryptography.RandomNumberGenerator]::Create().GetBytes($bytes)
    $key = -join ($bytes | ForEach-Object { $_.ToString("x2") })
    [System.IO.File]::WriteAllText($keyFile, $key + "`r`n", [System.Text.UTF8Encoding]::new($false))
    Write-Ok "Generated API key -> $keyFile"
    return $key
}

function Write-ClientConfigJs([string]$root, [string]$apiKey) {
    $js = @"
/* Generated by install.ps1 — do not commit. Shared secret for mutating CmApi routes. */
window.__PULSE_API_KEY__ = "$apiKey";
"@
    $p = Join-Path $root "config.local.js"
    [System.IO.File]::WriteAllText($p, $js + "`r`n", [System.Text.UTF8Encoding]::new($false))
    Write-Ok "Wrote $p (frontend X-Api-Key)"
}
...
function Set-JsonAppSettings([string]$root, [string]$pythonExe) {
    ...
    $apiKey = Get-OrCreateApiKey -root $root
    Write-ClientConfigJs -root $root -apiKey $apiKey
    $payload = @{
        Logging = @{
            LogLevel = @{
                Default = "Warning"
...
try {
    ...
    Write-Host "  API key:     data_live\cm_api_key.txt  (+ config.local.js for the UI)"
    Write-Host ""
